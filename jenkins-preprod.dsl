...
pipeline {
    ...
    stage('CF Map Real Route QA (KS1)') {
          environment{
               CF_HOME = '${spaceQa}'
          }
          steps{
            script {
             ...
             sh "cf --version"
             sh "cf login -a ${pcfUrlPreprod} -o ${cfOrgPreprod} -s ${spaceQa} -u $USERNAME -p $PASSWORD"
             sh "cf push ${appName} -f manifest.yml --vars-file vars-ks1-qa.yml"
             }
          }
    }
   }
    ...
    stage('CF Map Real Route QA (SH8)') {
            environment{
                CF_HOME = '${spaceQa}'
            }
            steps{
               withCredentials([usernamePassword(credentialsId: cfCredentialsIdPreprod, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
               sh "cf --version"
               sh "cf login -a ${pcfUrlPreprod2} -o ${cfOrgPreprod2} -s ${spaceQa} -u $USERNAME -p $PASSWORD"
               sh "cf push ${appName} -f manifest.yml --vars-file vars-sh8-qa.yml"
               }
            }
    }
   stage('QA Integration Test SH8') {
                               steps {
                                  withCredentials([string(credentialsId: "PREPROD_ADFS_CLIENT_SECRET", variable: "client_secret")]) {
                                   sh 'export;which node;node -v; npm -v'
                                   sh 'npm install -g newman'
                                   sh 'echo "QA Integration Test SH8"'
                                   sh 'newman -version'
                                   sh 'newman run newman/' + newmanFilePrefix + '-QA.postman_collection.json -e newman/' + newmanFilePrefix + '-QA-sh8.postman_environment.json --env-var "adfs_qa_client_secret=$client_secret"'
                               }
                        }
           }

   stage('QA Integration Test KS1') {
                            steps {
                            script {
                                 tee(INTEGRATION_STAGE_LOG_FILE) {
                                    withCredentials([string(credentialsId: "PREPROD_ADFS_CLIENT_SECRET", variable: "client_secret")]) {
                                        sh 'echo "QA Integration Test KS1"'
                                        sh 'newman -version'
                                        sh 'newman run newman/' + newmanFilePrefix + '-QA.postman_collection.json -e newman/' + newmanFilePrefix + '-QA-ks1.postman_environment.json --env-var "adfs_qa_client_secret=$client_secret"'
                            }
                         }
                     }
