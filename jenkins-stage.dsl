...
pipeline {
    ...
    stage('CF Map Real Route Stage (KS1)') {
          environment{
               CF_HOME = '${spaceProd}'
          }
          steps{
            script {
             ...
             sh "cf push ${appName} -f manifest-prod.yml --vars-file vars-ks1-stage.yml"
             }
          }
          }
          }
    ...
    stage('CF Map Real Route Stage (SH8)') {
            environment{
                CF_HOME = '${spaceProd}'
            }
            steps{
               ...
               sh "cf push ${appName} -f manifest-prod.yml --vars-file vars-sh8-stage.yml"
               }
            }
    }
